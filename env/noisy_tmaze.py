...
class MazeEnvironment(BaseEnvironment):
    ...
    def env_init(self, env_info={}):
        ...
        torch.manual_seed(env_info.get("seed", 47))
    ...
    def get_state_features(self, state):
        ...
        state = torch.eye(self.num_states)[self.get_observation(state)]

        bit = torch.Tensor([bit])
        state_feature = torch.cat([state, bit])
        state_feature += torch.randn(state_feature.shape) * 0.1
        return state_feature[None, ...].to(device)
